...
fs, signal = wavfile.read("Coldplay - Viva La Vida(40sec).wav")

with sf.SoundFile("Coldplay - Viva La Vida(40sec).wav", 'r') as f:
    num_frames = len(f)
    sample_rate = f.samplerate
    # Select a chunk from the file (in seconds)
    frames_to_read = int(40*sample_rate)
...
for i in range(num_mics):
    audio_outputEcho = Audio(room.mic_array.signals[i, :],rate=fs)
    with open("audio_4mic" + str(i) + ".wav", "wb") as f:
        f.write(audio_outputEcho.data)
...
# Example usage
file_paths = ['audio_4mic0.wav',
              'audio_4mic1.wav',
              'audio_4mic2.wav',
              'audio_4mic3.wav']

output_path = 'output_moving_sound_song_4ch_40sec.wav'
